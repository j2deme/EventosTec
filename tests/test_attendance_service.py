...
def test_calculate_attendance_percentage_partial_attendance(
    app, setup_attendance_test_data
):
...
def test_calculate_attendance_percentage_with_pauses(app, setup_attendance_test_data):
    ...
    with app.app_context():
        ...
        attendance.pause_time = datetime(
            2024, 1, 1, 10, 20, 0, tzinfo=timezone.utc
        )  # Pausa a los 20 mins
        attendance.resume_time = datetime(
            2024, 1, 1, 10, 35, 0, tzinfo=timezone.utc
        )  # Reanuda a los 35 mins
        attendance.check_out_time = datetime(
            2024, 1, 1, 11, 0, 0, tzinfo=timezone.utc
        )  # Termina a la 1hr
        db.session.commit()
