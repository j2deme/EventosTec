...
def test_preregistrations_by_career_no_filters(client, auth_headers, sample_data, app):
    ...
    with app.app_context():
        ...
        reg2 = Registration(
            student_id=student2.id, activity_id=activity.id, status="Registrado"
        )
        db.session.add_all([reg1, reg2])
        db.session.commit()

    response = client.get(
        "/api/reports/preregistrations_by_career", headers=auth_headers
    )
...
def test_preregistrations_by_career_filter_by_event(
    ...
    response = client.get(
        f"/api/reports/preregistrations_by_career?event_id={event1_id}",
...
def test_preregistrations_by_career_filter_by_activity(
    client, auth_headers, sample_data, app
):
    ...
    response = client.get(
        f"/api/reports/preregistrations_by_career?activity_id={activity_id}",
        headers=auth_headers,
    )
...
def test_preregistrations_by_career_groups_by_career_and_generation(
    client, auth_headers, sample_data, app
):
    ...
    with app.app_context():
        ...
        students = [
            Student(
                control_number="20241001",
                full_name="S1",
                email="s1@test.com",
                career="ISC",
            ),
            Student(
                control_number="20241002",
                full_name="S2",
                email="s2@test.com",
                career="ISC",
            ),
            Student(
                control_number="20231003",
                full_name="S3",
                email="s3@test.com",
                career="IIA",
            ),
        ...
        for s in students:
            reg = Registration(
                student_id=s.id, activity_id=activity.id, status="Registrado"
            )
            db.session.add(reg)
        db.session.commit()
