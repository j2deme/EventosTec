...
def test_create_generates_slug(monkeypatch):
    ...
    monkeypatch.setattr('app.services.activity_service.generate_unique_slug',
...
def test_update_name_without_apply_keeps_manual_slug(monkeypatch):
    ...
    monkeypatch.setattr(
        'app.services.activity_service.db.session', MagicMock())
    monkeypatch.setattr(
...
def test_update_name_with_apply_replaces_slug(monkeypatch):
    ...
    updated = update_activity(
        2, {'name': 'New Name', 'apply_generated_slug': True})
