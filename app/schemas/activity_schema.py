...
class ActivitySchema(ma.SQLAlchemyAutoSchema):
    ...
    department = fields.Str(required=True, validate=validate.Length(min=1, max=50))
    ...
    modality = fields.Str(
        required=True, validate=validate.OneOf(["Presencial", "Virtual", "Híbrido"])
    )
    requirements = fields.Str(load_default=None)
    max_capacity = fields.Int(load_default=None, validate=validate.Range(min=0))
    # Nuevos campos
    speakers = fields.List(fields.Dict(), load_default=None)
    # { general: bool, careers: [..] }
    target_audience = fields.Dict(load_default=None)
    knowledge_area = fields.Str(load_default=None, validate=validate.Length(max=100))
    ...
    current_capacity = fields.Method("get_current_capacity", dump_only=True)
            ...
            try:
                ...
                if ev is not None:
                    setattr(
                        ...
                        "event",
                        SimpleNamespace(
                            id=getattr(ev, "id", None), name=getattr(ev, "name", None)
                        ),
                    )
            except Exception:
                pass
            ...
            try:
                ...
                if ta is None:
                    ...
                    try:
                        ...
                        careers = [
                            x.strip() for x in (ta or "").split(",") if x.strip()
                        ]
                        setattr(
                            result,
                            "target_audience",
            ...
            if isinstance(ta, str):
                ...
                if t == "":
                    ...
                    try:
                        ...
                        careers = [x.strip() for x in t.split(",") if x.strip()]
                        data["target_audience"] = {"general": False, "careers": careers}
