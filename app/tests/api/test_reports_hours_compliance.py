...
def test_hours_compliance_report_all_students(
    client, auth_headers, sample_event_with_activities
):
    """Test hours compliance report returns all students."""
    event_id = sample_event_with_activities["event_id"]

    response = client.get(
        f"/api/reports/hours_compliance?event_id={event_id}", headers=auth_headers
    )
...
def test_hours_compliance_report_min_hours_filter(
    client, auth_headers, sample_event_with_activities
):
...
def test_hours_compliance_report_career_filter(
    client, auth_headers, sample_event_with_activities
):
...
def test_hours_compliance_report_search_control_number(
    client, auth_headers, sample_event_with_activities
):
...
def test_hours_compliance_report_search_name(
    client, auth_headers, sample_event_with_activities
):
...
def test_hours_compliance_report_missing_event_id(client, auth_headers):
    ...
    response = client.get("/api/reports/hours_compliance", headers=auth_headers)
...
def test_hours_compliance_report_invalid_event_id(client, auth_headers):
    ...
    response = client.get(
        "/api/reports/hours_compliance?event_id=99999", headers=auth_headers
    )
...
def test_hours_compliance_excel_generation(
    client, auth_headers, sample_event_with_activities
):
    ...
    response = client.get(
        f"/api/reports/hours_compliance_excel?event_id={event_id}", headers=auth_headers
    )
...
def test_student_participations_details(
    client, auth_headers, sample_event_with_activities
):
...
def test_student_participations_missing_event_id(
    client, auth_headers, sample_event_with_activities
):
    ...
    response = client.get(
        f"/api/reports/student_participations/{student1_id}", headers=auth_headers
    )

    assert response.status_code == 400


def test_student_participations_invalid_student(
    client, auth_headers, sample_event_with_activities
):
...
def test_hours_compliance_excludes_cancelled_registrations(
    client, auth_headers, sample_event_with_activities
):
...
def test_hours_compliance_report_unauthorized(client, sample_event_with_activities):
    ...
    response = client.get(f"/api/reports/hours_compliance?event_id={event_id}")
