...
def test_register_creates_related_attendances(app, client, auth_headers, sample_data):
    ...
    with app.app_context():
        ...
        assert Attendance.query.filter_by(
            student_id=student_id, activity_id=activity_b.id).first() is None
    ...
    payload = {'student_id': student_id,
               'activity_id': activity_a_id, 'mark_present': True}
    res = client.post('/api/attendances/register',
    ...
    with app.app_context():
        ...
        att_b = Attendance.query.filter_by(
            student_id=student_id, activity_id=activity_b_id).first()
