...
def test_check_out_calculates_percentage_and_syncs_registration(client, app, sample_data, auth_headers):
    ...
    resp = client.post('/api/attendances/check-out',
    ...
    with app.app_context():
        a = db.session.query(Attendance).filter_by(
...
def test_student_cannot_call_check_in(client, app, sample_data):
    ...
    payload = {'student_id': student_id, 'activity_id': activity_id}
    resp = client.post('/api/attendances/check-in',
