...
def create_student_and_token(app, client, control_number='20250001', full_name='Alumno Test'):
...
def test_student_sees_only_their_attendances(app, client):
    ...
    resp = client.get('/api/attendances/', headers=headers,
...
def test_admin_can_view_all_attendances_and_pause_resume_endpoints(app, client, auth_headers):
    ...
    with app.app_context():
        ...
        resp_pause = client.post('/api/attendances/pause', json={
                                 'student_id': s1.id, 'activity_id': a1.activity_id}, headers=auth_headers)
        ...
        resp_resume = client.post('/api/attendances/resume', json={
                                  'student_id': s1.id, 'activity_id': a1.activity_id}, headers=auth_headers)
